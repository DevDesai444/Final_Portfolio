...
import { doesSurveyActivateByAction, doesSurveyActivateByEvent, isSurveyRunning, SURVEY_LOGGER as logger, } from '../utils/survey-utils';
...
import { canActivateRepeatedly, createShadow, defaultSurveyAppearance, dismissedSurveyEvent, doesSurveyDeviceTypesMatch, doesSurveyMatchSelector, doesSurveyUrlMatch, getContrastingTextColor, getDisplayOrderQuestions, getSurveyResponseKey, getSurveySeen, hasWaitPeriodPassed, sendSurveyEvent, style, SURVEY_DEFAULT_Z_INDEX, SurveyContext, } from './surveys/surveys-extension-utils';
...
var SurveyManager = /** @class */ (function () {
    function SurveyManager(posthog) {
        ...
        this._canShowNextEventBasedSurvey = function () {
            var _a;
        ...
        this._handlePopoverSurvey = function (survey) {
            ...
            if (!surveySeen) {
                ...
                _this._addSurveyToFocus(survey.id);
                var delaySeconds = ((_b = survey.appearance) === null || _b === void 0 ? void 0 : _b.surveyPopupDelaySeconds) || 0;
                var shadow_1 = createShadow(style(survey === null || survey === void 0 ? void 0 : survey.appearance), survey.id, undefined, _this._posthog);
                if (delaySeconds <= 0) {
                    return Preact.render(<SurveyPopup key={'popover-survey'} posthog={_this._posthog} survey={survey} removeSurveyFromFocus={_this._removeSurveyFromFocus} isPopup={true}/>, shadow_1);
                }
                var timeoutId = setTimeout(function () {
                    if (!doesSurveyUrlMatch(survey)) {
                        return _this._removeSurveyFromFocus(survey.id);
                    }
                    // rendering with surveyPopupDelaySeconds = 0 because we're already handling the timeout here
                    Preact.render(<SurveyPopup key={'popover-survey'} posthog={_this._posthog} survey={__assign(__assign({}, survey), { appearance: __assign(__assign({}, survey.appearance), { surveyPopupDelaySeconds: 0 }) })} removeSurveyFromFocus={_this._removeSurveyFromFocus} isPopup={true}/>, shadow_1);
                }, delaySeconds * 1000);
        ...
        this._handleWidget = function (survey) {
            ...
            var shadow = retrieveWidgetShadow(survey, _this._posthog);
            var stylesheetContent = style(survey.appearance);
            var WIDGET_STYLE_ID = 'ph-data-style-id';
            // Check if the stylesheet already exists
            if (!shadow.querySelector("style[".concat(WIDGET_STYLE_ID, "=\"").concat(WIDGET_STYLE_ID, "\"]"))) {
                var stylesheet = prepareStylesheet(document, stylesheetContent, _this._posthog);
                if (stylesheet) {
            ...
            Preact.render(<FeedbackWidget key={'feedback-survey-' + survey.id} // Use unique key
             posthog={_this._posthog} survey={survey} removeSurveyFromFocus={_this._removeSurveyFromFocus}/>, shadow);
        };
        this._removeWidgetSelectorListener = function (surveyId) {
        ...
        this._manageWidgetSelectorListener = function (survey) {
            var _a;
            ...
            if (!currentElement) {
                ...
                if (existingListenerData) {
                    _this._removeWidgetSelectorListener(survey.id);
                }
                return;
            }
            ...
            if (!currentElement.hasAttribute('PHWidgetSurveyClickListener')) {
                ...
                _this._widgetSelectorListeners.set(survey.id, { element: currentElement, listener: listener });
                logger.info("Attached click listener for survey ".concat(survey.id));
            }
        };
        this.renderSurvey = function (survey, selector) {
            Preact.render(<SurveyPopup key={'popover-survey'} posthog={_this._posthog} survey={survey} removeSurveyFromFocus={_this._removeSurveyFromFocus} isPopup={false}/>, selector);
        };
        ...
        this.callSurveysAndEvaluateDisplayLogic = function (forceReload) {
            ...
            _this.getActiveMatchingSurveys(function (surveys) {
                var nonAPISurveys = surveys.filter(function (survey) { return survey.type !== SurveyType.API; });
                ...
                var nonAPISurveyQueue = _this._sortSurveysByAppearanceDelay(nonAPISurveys);
                ...
                nonAPISurveyQueue.forEach(function (survey) {
                    ...
                    if (survey.type === SurveyType.Widget) {
                        if (((_a = survey.appearance) === null || _a === void 0 ? void 0 : _a.widgetType) === SurveyWidgetType.Tab) {
                            ...
                            _this._manageWidgetSelectorListener(survey);
                        }
                    }
                    // Popover Type Logic (only one shown at a time)
                    if (isNull(_this._surveyInFocus) &&
                        survey.type === SurveyType.Popover &&
                        _this._canShowNextEventBasedSurvey()) {
                        _this._handlePopoverSurvey(survey);
                    }
                });
                // Clean up listeners for surveys that are no longer active or matched
                _this._widgetSelectorListeners.forEach(function (_, surveyId) {
                    if (!activeSelectorSurveyIds.has(surveyId)) {
                        _this._removeWidgetSelectorListener(surveyId);
                    }
                });
            }, forceReload);
        };
        this._addSurveyToFocus = function (id) {
            if (!isNull(_this._surveyInFocus)) {
                logger.error("Survey ".concat(__spreadArray([], __read(_this._surveyInFocus), false), " already in focus. Cannot add survey ").concat(id, "."));
            }
            _this._surveyInFocus = id;
        };
        this._removeSurveyFromFocus = function (id) {
            if (_this._surveyInFocus !== id) {
                logger.error("Survey ".concat(id, " is not in focus. Cannot remove survey ").concat(id, "."));
            }
            _this._clearSurveyTimeout(id);
    ...
    SurveyManager.prototype._checkFlags = function (survey) {
        ...
        return survey.feature_flag_keys.every(function (_a) {
            ...
            return _this._posthog.featureFlags.isFeatureEnabled(value);
        });
    };
...
export function FeedbackWidget(_a) {
    ...
    return (<Preact.Fragment>
            {((_e = survey.appearance) === null || _e === void 0 ? void 0 : _e.widgetType) === 'tab' && (<div className="ph-survey-widget-tab" onClick={function () { return !readOnly && setShowSurvey(!showSurvey); }} style={{ color: getContrastingTextColor(survey.appearance.widgetColor) }}>
