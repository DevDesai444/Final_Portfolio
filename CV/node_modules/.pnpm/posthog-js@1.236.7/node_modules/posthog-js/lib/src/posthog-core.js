...
import { _copyAndTruncateStrings, addEventListener, each, eachArray, extend, isCrossDomainCookie, safewrapClass, } from './utils';
...
import { isArray, isEmptyObject, isEmptyString, isError, isFunction, isKnownUnsafeEditableEvent, isNullish, isNumber, isObject, isString, isUndefined, } from './utils/type-utils';
...
var PostHog = /** @class */ (function () {
    ...
    PostHog.prototype._init = function (token, config, name) {
        ...
        this._requestQueue = new RequestQueue(function (req) { return _this._send_retriable_request(req); }, this.config.request_queue_config);
        ...
        if (((_b = config.bootstrap) === null || _b === void 0 ? void 0 : _b.distinctID) !== undefined) {
            ...
            this.persistence.set_property(USER_STATE, ((_d = config.bootstrap) === null || _d === void 0 ? void 0 : _d.isIdentifiedID) ? 'identified' : 'anonymous');
            this.register({
    ...
    PostHog.prototype._calculate_event_properties = function (event_name, event_properties, timestamp, uuid) {
        var _a;
        ...
        var infoProperties = getEventProperties(this.config.mask_personal_data_properties, this.config.custom_personal_data_properties);
    ...
    PostHog.prototype._calculate_set_once_properties = function (dataSetOnce) {
        ...
        var setOnceProperties = extend({}, initialProps, sessionProps || {}, dataSetOnce || {});
        var sanitize_properties = this.config.sanitize_properties;
        if (sanitize_properties) {
            logger.error('sanitize_properties is deprecated. Use before_send instead');
    ...
    PostHog.prototype.setPersonProperties = function (userPropertiesToSet, userPropertiesToSetOnce) {
        ...
        var hash = getPersonPropertiesHash(this.get_distinct_id(), userPropertiesToSet, userPropertiesToSetOnce);
    ...
    PostHog.prototype.set_config = function (config) {
        ...
        if (isObject(config)) {
            ...
            if (this.config.debug) {
                Config.DEBUG = true;
                logger.info('set_config', JSON.stringify({
                    config: config,
                    oldConfig: oldConfig,
    ...
    PostHog.prototype.captureException = function (error, additionalProperties) {
        ...
        this.exceptions.sendExceptionEvent(__assign(__assign({}, errorToProperties(isError(error) ? { error: error, event: error.message } : { event: error }, 
    ...
    PostHog.prototype._runBeforeSend = function (data) {
        ...
        var fns = isArray(this.config.before_send) ? this.config.before_send : [this.config.before_send];
            ...
            for (var fns_1 = __values(fns), fns_1_1 = fns_1.next(); !fns_1_1.done; fns_1_1 = fns_1.next()) {
                ...
                if (isNullish(beforeSendResult)) {
                    ...
                    if (isKnownUnsafeEditableEvent(data.event)) {
                        logger.warn("".concat(logMessage, ". This can cause unexpected behavior."));
                    }
                    else {
                ...
                if (!beforeSendResult.properties || isEmptyObject(beforeSendResult.properties)) {
                    logger.warn("Event '".concat(data.event, "' has no properties after beforeSend function, this is likely an error."));
                }
            }
        }
